...
const translations = {
  en: {
    ...
    formatCardDesc: "Import a folder, JSON file, or atlas directly, with automatic recovery for standard Codex v1/v2 layouts.",
    workflowIndex: "02 / Workflow",
    workflowTitle: "From atlas to desktop<br />in three simple moves.",
    importTitle: "Import a project",
    importDesc: "Choose a project folder, JSON manifest, or spritesheet. The app discovers sibling files and can recover standard Codex v1/v2 layouts from the atlas alone.",
    composeTitle: "Compose animations",
    composeDesc: "Tune frames and playback, preview any full action once at top priority, then choose its trigger and optional delay.",
    ...
    formatBadge: "Codex v1/v2 compatible",
  ...
  zh: {
    ...
    formatCardDesc: "文件夹、JSON 或图集都能直接导入，并可自动恢复标准 Codex v1/v2 图集布局。",
    workflowIndex: "02 / 工作流",
    workflowTitle: "从图集到桌面，<br />只有三个动作。",
    importTitle: "导入工程",
    importDesc: "选择工程文件夹、JSON 配置或图集均可；应用会自动发现同目录文件，也能只凭图集恢复标准 Codex v1/v2 布局。",
    composeTitle: "编排动作",
    composeDesc: "逐帧微调并设置播放规则，以最高优先级完整预览一遍，再为动作选择触发事件和可选延迟。",
    desktopTitle: "放到桌面",
    desktopDesc: "开启工程显示；每只桌宠独立运行、移动并响应自己的事件。",
    formatIndex: "03 / 工程格式",
    formatTitle: "透明、可读，<br />没有隐藏的素材库。",
    formatIntro: "导出的工程就是 JSON 配置加一张完整 PNG，易于备份，也方便继续交给其他工具。",
    formatBadge: "兼容 Codex v1/v2",
...
const updateRelease = async () => {
  ...
  try {
    ...
    document.querySelectorAll("[data-version]").forEach((node) => {
      node.textContent = release.tag_name || "v0.6.0";
    });
